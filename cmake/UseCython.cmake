...
# using cython.
#
# Cython_transpile(<pyx_file> [LANGUAGE C | CXX] [CYTHON_ARGS <args> ...]
# [OUTPUT <OutputFile>] [OUTPUT_VARIABLE <OutputVariable>])
#
# Options:
#
# ``LANGUAGE [C | CXX]`` Force the generation of either a C or C++ file.
# Recommended; will attempt to be deduced if not specified, defaults to C unless
# only CXX is enabled.
#
# ``CYTHON_ARGS <args>`` Specify additional arguments for the cythonization
# process. Will default to the ``CYTHON_ARGS`` variable if not specified.
#
# ``OUTPUT <OutputFile>`` Specify a specific path for the output file as
# ``<OutputFile>``. By default, this will output into the current binary dir. A
# depfile will be created alongside this file as well.
#
# ``OUTPUT_VARIABLE <OutputVariable>`` Set the variable ``<OutputVariable>`` in
# the parent scope to the path to the generated source file.
#
# Defined variables:
#
# ``<OutputVariable>``
...
#
# Cython_transpile(_hello.pyx OUTPUT_VARIABLE _hello_source_files )
#
# Python_add_library(_hello MODULE ${_hello_source_files} WITH_SOABI )
#
# =============================================================================
...
if(CMAKE_VERSION VERSION_LESS "3.8")
  # CMake 3.7 required for DEPFILE CMake 3.8 required for COMMAND_EXPAND_LISTS
...
function(Cython_transpile)
  ...
  cmake_parse_arguments(_args "${_options}" "${_one_value}" "${_multi_value}"
                        ${ARGN})
  ...
  if(NOT input_length EQUAL 1)
    message(
      FATAL_ERROR
        "One and only one input file must be specified, got '${_source_files}'")
  endif()
  ...
  function(_transpile _source_file generated_file language)
    ...
    file(RELATIVE_PATH generated_file_relative ${CMAKE_BINARY_DIR}
         ${generated_file})
    file(RELATIVE_PATH source_file_relative ${CMAKE_SOURCE_DIR} ${_source_file})
    set(comment
        "Generating ${_language} source '${generated_file_relative}' from '${source_file_relative}'"
    )
    ...
    add_custom_command(
      ...
      COMMAND ${_cython_command} ${_language_arg} "${_args_CYTHON_ARGS}"
              ${_depfile_arg} ${pyx_location} --output-file ${generated_file}
  ...
  function(_set_output _input_file _language _output_var)
    ...
    # Can use cmake_path for CMake 3.20+ cmake_path(GET _input_file STEM
    # basename)
    get_filename_component(_basename "${_input_file}" NAME_WE)

    if(IS_ABSOLUTE ${_input_file})
      file(RELATIVE_PATH _input_relative ${CMAKE_CURRENT_SOURCE_DIR}
           ${_input_file})
    else()
    ...
    set(${_output_var}
        "${CMAKE_CURRENT_BINARY_DIR}/${_output_relative_dir}${_basename}.${_language_extension}"
        PARENT_SCOPE)
  endfunction()
  ...
  if(NOT _language)
    ...
    elseif("CXX" IN_LIST _languages)
      ...
      message(
        FATAL_ERROR
          "LANGUAGE keyword required if neither C nor CXX enabled globally")
    endif()
  endif()
...
function(_cython_compute_language OUTPUT_VARIABLE FILENAME)
  ...
  # Check for compiler directive similar to "# distutils: language = c++" See
  # https://cython.readthedocs.io/en/latest/src/userguide/wrapping_CPlusPlus.html#declare-a-var-with-the-wrapped-c-class
  set(REGEX_PATTERN
      [=[^[[:space:]]*#[[:space:]]*distutils:.*language[[:space:]]*=[[:space:]]*(c\\+\\+|c)]=]
  )
